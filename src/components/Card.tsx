...
const Card = ({ content }: IContent) => {
  return (
    <Link to={`/video-detail/${content?.video?.videoId}`} className="hover:scale-[1.01] flex flex-col items-center w-[280px] min-h-[300px] relative">
      <LazyLoadImage effect="blur" loading="lazy" height={180} width={280} placeholderSrc={content?.video?.thumbnails[0].url} alt={content?.video?.title} className=" rounded-t-lg" src={content?.video?.thumbnails[0].url || undefinedImage} />
